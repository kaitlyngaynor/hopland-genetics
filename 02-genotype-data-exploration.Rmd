...
pdf(here::here("figures", "storage-number-working.pdf"), width = 4, height = 5)
ggplot(genotypes, aes(x = n, fill = stor)) +
  geom_histogram() +
  facet_grid(stor ~ .) +
  ylab("Number of Samples") +
...
#### FIG condition-genotype-complete.pdf
```{r condition-genotype-complete}
# scale to percentage so it's easier to compare across sample types
pdf(here::here("figures", "condition-genotype-complete.pdf"), width = 7, height = 5)
ggplot(perfect.storage.cond.coarse, aes(x = cond.coarse, y = n, fill = Complete)) +
  geom_bar(stat = "identity", position = "fill") +
  facet_wrap(~ stor) +
