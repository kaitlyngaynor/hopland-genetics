...
DO NOT USE - it maybe doesn't make sense to have "working" as the independent variable and DNA concentration as the dependent variable, causality is a bit inappropriate
```{r glmm qpcr}
# no variables—just random effect
fit0 <- lmer(log(extracted_conc) ~ (1 | lab_id), data = genotypes)
summary(fit0)
AIC(fit0) # 1200.9

# effect of condition only
fit1 <- lmer(log(extracted_conc) ~ cond + (1 | lab_id), data = genotypes)
summary(fit1)
AIC(fit1) # 1125.9

# effect of condition only (coarse categories)
fit1.5 <- lmer(log(extracted_conc) ~ cond.coarse + (1 | lab_id), data = genotypes)
summary(fit1.5)
AIC(fit1.5) # 1125.2

# effect of storage only
fit2 <- lmer(log(extracted_conc) ~ stor + (1 | lab_id), data = genotypes)
summary(fit2) 
AIC(fit2) # 1192.9

# effect of storage and condition
fit3 <- lmer(log(extracted_conc) ~ stor + cond + (1 | lab_id), data = genotypes)
summary(fit3)
AIC(fit3) # 1117.3

# effect of storage and condition (interacting)
fit4 <- lmer(log(extracted_conc) ~ stor * cond + (1 | lab_id), data = genotypes)
summary(fit4)
AIC(fit4) # 1117.1 *** BEST
```
